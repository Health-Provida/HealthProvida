/**
 * ProviderLayout.jsx
 * Shell layout for provider dashboard pages — sidebar nav + top bar + content area.
 * Uses a teal/green accent theme to distinguish from the admin panel.
 */
import React, { useState } from 'react';
import { NavLink, Link, useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import {
  LayoutDashboard, Building2, Calendar, Star, Clock,
  Settings, LogOut, ChevronLeft, ChevronRight, Menu, X,
  Bell, Home, Stethoscope, BarChart3, Users, MessageSquare
} from 'lucide-react';
import { useAuth } from '@/context/AuthContext';

const NAV_ITEMS = [
  { to: '/provider/dashboard',    icon: LayoutDashboard, label: 'Dashboard' },
  { to: '/provider/clinic',       icon: Building2,       label: 'My Clinic' },
  { to: '/provider/appointments', icon: Calendar,        label: 'Appointments', badgeKey: 'pendingAppts' },
  { to: '/provider/reviews',      icon: Star,            label: 'Reviews' },
  { to: '/provider/schedule',     icon: Clock,           label: 'Schedule' },
  { to: '/provider/analytics',    icon: BarChart3,       label: 'Analytics' },
  { to: '/provider/staff',        icon: Users,           label: 'Staff' },
  { to: '/provider/messages',     icon: MessageSquare,   label: 'Messages', badgeKey: 'unreadMessages' },
  { to: '/provider/settings',     icon: Settings,        label: 'Settings' },
];

export default function ProviderLayout({ children, badges = {} }) {
  ...
  const navItemClass = ({ isActive }) => {
    const base = 'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group relative';
    if (isActive) {
      return `${base} bg-gradient-to-r from-teal-600/20 to-green-600/10 text-white`;
    }
        ...
        {!collapsed && (
          <Link to="/provider/dashboard" className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">Provider Dashboard</p>
            </div>
          </Link>
        )}
        {collapsed && (
            ...
            >
              ...
              {!collapsed && (
                <>
                  ...
                  {badge > 0 && (
                    <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-500 text-white min-w-[20px] text-center">
                      {badge > 99 ? '99+' : badge}
                    </span>
                  )}
                </>
      ...
      <div className="px-3 pb-4 border-t border-white/5 pt-3">
        {!collapsed && (
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-teal-500 to-green-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
              {profile?.full_name?.charAt(0)?.toUpperCase() || 'P'}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-white truncate">{profile?.full_name || 'Provider'}</p>
